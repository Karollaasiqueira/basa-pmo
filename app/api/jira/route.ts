...
function classify(s: string): 'done'|'doing'|'blocked'|'analysis'|'cancelled'|'review'|'todo' {
  const n = s.toLowerCase();
  if (DONE_KEYWORDS.some(k => n.includes(k)))     return 'done';
  if (CANCEL_KEYWORDS.some(k => n.includes(k)))   return 'cancelled';
  if (BLOCKED_KEYWORDS.some(k => n.includes(k)))  return 'blocked';
  if (REVIEW_KEYWORDS.some(k => n.includes(k)))   return 'review';
  if (DOING_KEYWORDS.some(k => n.includes(k)))    return 'doing';
  if (ANALYSIS_KEYWORDS.some(k => n.includes(k))) return 'analysis';
  return 'todo';
}

function getDrop(labels: string[]): string | null {
  for (const l of labels) { const d = DROP_LABEL_MAP[l.toLowerCase()]; if (d) return d; }
  return null;
}

function getParceiro(labels: string[]): string | null {
  for (const l of labels) { const p = PARCEIRO_LABEL_MAP[l.toLowerCase()]; if (p) return p.nome; }
  return null;
}
...
const FIELDS = [
  ...
  'customfield_10020','customfield_10014','customfield_10016',
];

async function getAllIssues(jql: string): Promise<any[]> {
  const all: any[] = [];
  ...
  do {
    ...
    if (!res.ok) {
      ...
      throw new Error(`JIRA_ERROR ${res.status}: ${text.slice(0, 500)}`);
    }
  ...
  } while (nextPageToken && all.length < 2000);
...
async function getLastComment(key: string) {
  try {
    ...
    return {
      ...
      data: new Date(c.created).toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' }),
  ...
  try {
    if (!BASE_URL || !EMAIL || !API_TOKEN) {
      return NextResponse.json({
        ...
        BASE_URL: BASE_URL ? 'ok' : 'FALTANDO',
        EMAIL: EMAIL ? 'ok' : 'FALTANDO',
        API_TOKEN: API_TOKEN ? 'ok' : 'FALTANDO',
      }, { status: 500 });
    }
    ...
    if (action === 'me') {
      const res = await fetch(`${BASE_URL}/rest/api/3/myself`, { headers: HEADERS(), cache: 'no-store' });
      const data = await res.json();
      return NextResponse.json({ status: res.status, user: data });
    }

    if (action === 'projects') {
      const res = await fetch(`${BASE_URL}/rest/api/3/project/search?maxResults=50`, { headers: HEADERS(), cache: 'no-store' });
      const data = await res.json();
      return NextResponse.json({ projects: data.values ?? [], status: res.status });
    }
    ...
    if (allIssues.length === 0) {
      return NextResponse.json({ debug: true, message: 'Nenhuma issue encontrada', jql, base_url: BASE_URL, email: EMAIL, token_length: API_TOKEN?.length });
    }
    ...
    for (const issue of allIssues) {
      ...
      const parceiroInfo = parceiroNome ? Object.values(PARCEIRO_LABEL_MAP).find(p => p.nome === parceiroNome) : null;
      const isBug = BUG_TYPES.some(k => (issue.fields?.issuetype?.name ?? '').toLowerCase().includes(k));
      ...
      if (parceiroNome) {
        if (!parcMap[parceiroNome]) parcMap[parceiroNome] = {
          parceiro:parceiroNome, papel:parceiroInfo?.papel??'',
          drops:new Set(), total:0,concluido:0,emAndamento:0,bloqueado:0,backlog:0,cancelado:0,analise:0,
        };
      ...
      processed.push({
        ...
        status: statusName, statusCat: cat,
        tipo: issue.fields?.issuetype?.name ?? '', isBug,
        ...
        labels, drop, parceiro: parceiroNome,
    ...
    const paradosList = processed
      ...
      .sort((a, b) => b.diasParado - a.diasParado).slice(0, 20);

    const paradosComComentario = await Promise.all(
      paradosList.slice(0, 10).map(async i => ({ ...i, ultimoComentario: await getLastComment(i.key) }))
    );
    const paradosRest = paradosList.slice(10).map(i => ({ ...i, ultimoComentario: null }));

    const bugs = processed.filter(i => i.isBug && !['done','cancelled'].includes(i.statusCat))
      .sort((a, b) => b.diasParado - a.diasParado).slice(0, 50);

    const emRevisao = processed.filter(i => i.statusCat === 'review')
      .sort((a, b) => b.diasParado - a.diasParado).slice(0, 50);

    const porDrop = Object.entries(dropMap).map(([drop, d]) => ({
    ...
    const porSprint = Object.values(sprintMap)
      .sort((a: any, b: any) => a.nome.localeCompare(b.nome, undefined, { numeric: true }));

    return NextResponse.json({
      projectKey, geradoEm: new Date().toISOString(),
      summary, porDrop, porParceiro, porSprint,
      parados: [...paradosComComentario, ...paradosRest],
      bugs, emRevisao, totalIssues: allIssues.length,
    });
